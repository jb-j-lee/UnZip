...
public class ZipEncryptOutputStream extends OutputStream {
            ...
            switch (state) {
                ...
                case FLAGS:
                    ...
                    if (section == Section.LFH) {
                        ...
                        if ((b & 8) == 8) {
                            bufferUntil(State.FILE_BUFFERED, ZipUtil.CFH_SIGNATURE, ZipUtil.LFH_SIGNATURE);
                        }
                    }
                    b = b & 0xf7 | 1;
                    break;
                ...
                case FILE_HEADER_OFFSET:
                    ...
                    fileIndex++;
                    skipBytes = 3;
                    copyBytesUntil(State.SECTION_HEADER, ZipUtil.CFH_SIGNATURE, ZipUtil.ECD_SIGNATURE);
                    return;
    ...
    private static void adjustSize(int[] values) {
        int inc = ZipUtil.DECRYPT_HEADER_SIZE;
        for (int i = 0; i < 4; i++) {
    ...
    private void identifySectionHeader() throws IllegalStateException, IOException {
        if (Arrays.equals(buffer, ZipUtil.LFH_SIGNATURE)) {
            section = Section.LFH;
            copyBytes = 1;
            state = State.FLAGS;
            localHeaderOffset.add(bytesWritten);
        } else if (Arrays.equals(buffer, ZipUtil.CFH_SIGNATURE)) {
            section = Section.CFH;
            copyBytes = 3;
            state = State.FLAGS;
        ...
        } else if (Arrays.equals(buffer, ZipUtil.ECD_SIGNATURE)) {
            section = Section.ECD;
            copyBytes = 11;
    ...
    private void writeDecryptHeader() throws IOException {
        ...
        decryptHeader = new byte[ZipUtil.DECRYPT_HEADER_SIZE];
